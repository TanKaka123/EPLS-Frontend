    ...
    for (var i = 0; i < SCORES.length - 1; i++)
      for (var j = i + 1; j < SCORES.length; j++) {
        if (SCORES[i]["score"]["Pts"] < SCORES[j]["score"]["Pts"]) {
          ...
          SCORES[i]["score"]["Pts"] == SCORES[j]["score"]["Pts"] &&
          SCORES[i]["score"]["GD"] < SCORES[j]["score"]["GD"]
        ) {
          const flag = SCORES[i];
          SCORES[i] = SCORES[j];
          SCORES[j] = flag;
              ...
              SCORES.map((value, key) => {
                ...
                return (
                  <tr className="row-table-rank" key={key}>
                    ...
                    <td className="all-points hidden-device">
                      {value["score"]["F"]}
                    </td>
                    <td className="all-points hidden-device">
                      {value["score"]["A"]}
                    ...
                    <td className="rank-scores all-points">
                      {value["score"]["Pts"]}
                    </td>
                      ...
                      {Object &&
                        Object.keys(LastFiveGame).map(function (key, index) {
                          return (
                            <div key={index}>
                              {(LastFiveGame[key] == "w" && (
                                <img src={require("../../assets/images/wdl/w.png")} alt="w" />
                              )) ||
                                (LastFiveGame[key] == "l" && (
                                  <img src={require("../../assets/images/wdl/l.png")} alt="l" />
                                )) ||
                                (LastFiveGame[key] == "d" && (
                                  <img src={require("../../assets/images/wdl/d.png")} alt="d" />
                                ))}
                            </div>
                          );
                        })}
